...
function renderStops(){
  ...
  el.querySelectorAll('.stop:not(.hidden)').forEach(function(row){
    row.addEventListener('click',function(){var ll=stopLL(trip.stops[+row.getAttribute('data-i')]);if(!ll)return;if(window.matchMedia('(max-width:980px)').matches)setMobileView('map',true);if(mapOK&&map){map.panTo({lat:ll[0],lng:ll[1]});map.setZoom(Math.max(map.getZoom(),7));}});
...
function boot(){
  ...
  if(window.matchMedia('(max-width:980px)').matches)setMobileView('map');
...
function setMobileView(t,skipFit){
  document.body.classList.toggle('m-map', t==='map');
  document.body.classList.toggle('m-plan', t==='plan');
  ...
  if(t==='map'&&mapOK&&map&&typeof google!=='undefined'&&google.maps&&google.maps.event){
    google.maps.event.trigger(map,'resize');
    if(!skipFit)drawMap(true);
  }
